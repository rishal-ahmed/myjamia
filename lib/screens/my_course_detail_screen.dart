...
class MyCourseDetailScreen extends StatefulWidget {
  ...
  const MyCourseDetailScreen({Key? key, required this.courseId, required this.len, required this.enableDripContent}) : super(key: key);
...
class _MyCourseDetailScreenState extends State<MyCourseDetailScreen> with SingleTickerProviderStateMixin {
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      Provider.of<MyCourses>(context, listen: false).fetchCourseSections(widget.courseId).then((_) {
        final activeSections = Provider.of<MyCourses>(context, listen: false).sectionItems;
        setState(() {
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    final myLoadedCourse = Provider.of<MyCourses>(context, listen: false).findById(widget.courseId);
                        ...
                        child: Row(
                          children: [
                            ...
                            PopupMenuButton(
                              onSelected: (value) {
                                if (value == 'details') {
                                  Navigator.of(context).pushNamed(CourseDetailScreen.routeName, arguments: myLoadedCourse.id);
                                } else {
                        ...
                        child: Row(
                          children: [
                            ...
                            Padding(
                              padding: const EdgeInsets.only(bottom: 10.0),
                              child: CustomText(
                                text: '${myLoadedCourse.totalNumberOfCompletedLessons}/${myLoadedCourse.totalNumberOfLessons}',
                                fontSize: 14,
                                colors: Colors.grey,
                              ),
                            ),
                                  ...
                                  return InkWell(
                                    onTap: () {
                                      if (sections[0].id == section.id) {
                                        if (indexLess != 0) {
                                          if (section.mLesson![indexLess - 1].isCompleted != '1') {
                                            CommonFunctions.showWarningToast('previous lessons was not completed.');
                                          } else {
                                            setState(() {
                                        ...
                                        if (sections[index - 1].mLesson!.last.isCompleted != '1') {
                                          CommonFunctions.showWarningToast('previous lessons was not completed.');
                                        } else {
                                          setState(() {
                                            _activeLesson = lesson;
                                            ...
                                            children: <Widget>[
                                              sections[0].id == section.id
                                                  ? indexLess == 0
                                                      ...
                                                      : section.mLesson![indexLess - 1].isCompleted != '1'
                                                          ? const Expanded(
                                                              flex: 1,
                                                  ...
                                                  : sections[index - 1].mLesson!.last.isCompleted != '1'
                                                      ? const Expanded(
                                                          flex: 1,
                                                          child: Icon(Icons.lock_outlined),
                                                        )
                                              ...
                                              if (lesson.lessonType == 'video')
                                                Expanded(
                                                  ...
                                                  child: IconButton(
                                                    icon: const Icon(Icons.file_download_outlined),
                                                    color: Colors.black45,
                                                    onPressed: () => _initDownload(lesson, widget.courseId, myLoadedCourse.title,
                                                        myLoadedCourse.thumbnail, section.title, section.id),
                                                  ),
                                                ),
                                            ],
                                          ),
