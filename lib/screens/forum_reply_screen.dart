...
class _ForumReplyScreenState extends State<ForumReplyScreen> {
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      Provider.of<CourseForum>(context, listen: false).fetchForumChildQuestion(widget.questionId).then((_) {
        setState(() {
          _isLoading = false;
                  ...
                  if (activeQuestions.isNotEmpty)
                    ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          var cmDt = DateTime.fromMillisecondsSinceEpoch(int.parse(activeQuestions[index].dateAdded.toString()) * 1000);
                          // 12 Hour format:
                          var commentDate = DateFormat('dd MMM, yy').format(cmDt);
                          return Padding(
                                  ...
                                  children: [
                                    Padding(
                                      ...
                                      child: Align(
                                        ...
                                        child: CircleAvatar(
                                          radius: 20,
                                          backgroundImage: NetworkImage(activeQuestions[index].userImage.toString()),
                                          backgroundColor: Colors.grey,
                                        ),
                                      ),
                                    ),
                                          ...
                                          children: [
                                            Padding(
                                              ...
                                              child: Row(
                                                children: [
                                                  Text(
                                                    activeQuestions[index].userName.toString(),
                                                    style: const TextStyle(
                                                      fontSize: 15,
                                                      fontWeight: FontWeight.bold,
                                                    ),
                                                  ...
                                                  if (userId == activeQuestions[index].userId)
                                                    IconButton(
                                                      padding: EdgeInsets.zero,
                                                      constraints: const BoxConstraints(),
                                                      onPressed: () {
                                                        _delete(activeQuestions[index].id.toString(), index);
                                                      },
                                                      color: kRedColor,
                                                      iconSize: 20,
                                                      tooltip: 'Remove',
                                            ...
                                            Padding(
                                              padding: const EdgeInsets.only(right: 10, left: 15, top: 10.0),
                                              child: CustomText(
                                                text: activeQuestions[index].description.toString(),
                                                fontSize: 15,
                                              ),
                                            ),
                                          ],
