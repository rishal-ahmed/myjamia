...
import 'package:myjamia/widgets/ask_question_widget.dart';
import 'package:myjamia/widgets/forum_question_grid.dart';
...
class SearchForum extends StatefulWidget {
  ...
  const SearchForum({Key? key, required this.courseId, required this.keyWord}) : super(key: key);
...
class _SearchForumState extends State<SearchForum> {
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      Provider.of<CourseForum>(context, listen: false).fetchCourseSearchQuestions(widget.courseId, widget.keyWord).then((_) {
        setState(() {
          _isLoading = false;
  ...
  reset() async {
    ...
    await Provider.of<CourseForum>(context, listen: false).fetchCourseSearchQuestions(widget.courseId, _controller.text).then((_) {
      setState(() {
        _isLoading = false;
        activeQuestions = Provider.of<CourseForum>(context, listen: false).questions;
      });
  ...
  Widget searchBody() {
    if (_isInit && activeQuestions.isEmpty) {
      return SingleChildScrollView(
        child: SizedBox(height: MediaQuery.of(context).size.height * .5, child: const Center(child: CircularProgressIndicator())),
      );
    } else {
