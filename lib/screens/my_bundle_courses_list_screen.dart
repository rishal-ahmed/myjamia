...
import 'package:myjamia/providers/bundles.dart';
import 'package:myjamia/providers/shared_pref_helper.dart';
import 'package:myjamia/widgets/app_bar_two.dart';
import 'package:myjamia/widgets/my_bundle_course_list_item.dart';
...
class MyBundleCoursesListScreen extends StatefulWidget {
  ...
  @override
  State<MyBundleCoursesListScreen> createState() => _MyBundleCoursesListScreenState();
}
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      Provider.of<Bundles>(context).fetchBundleDetailById(bundleId).then((_) {
        setState(() {
          _isLoading = false;
          bundles = Provider.of<Bundles>(context, listen: false).getBundleDetail;
        });
      });
    }
    _isInit = false;
                        ...
                        children: [
                          ...
                          MaterialButton(
                            ...
                            onPressed: () async {
                              ...
                              final url = '$BASE_URL/api/web_redirect_to_buy_bundle/$authToken/$bundleId/academybycreativeitem';
                              _launchURL(url);
                            },
                              ...
                              itemBuilder: (ctx, index) {
                                return Center(
                                  child: MyBundleCourseListItem(
                                    ...
                                    id: int.parse(bundles[i].bundleCourses[index].id),
                                    title: bundles[i].bundleCourses[index].title,
                                    thumbnail: bundles[i].bundleCourses[index].thumbnail,
                                    rating: bundles[i].bundleCourses[index].rating,
                                    numberOfRatings: bundles[i].bundleCourses[index].numberOfRatings,
                                    price: bundles[i].bundleCourses[index].price,
                                    instructorName: bundles[i].bundleCourses[index].instructorName,
                                    instructorImage: bundles[i].bundleCourses[index].instructorImage,
                                  ),
                                );
                              },
                            ),
