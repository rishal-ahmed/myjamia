...
import 'package:myjamia/providers/my_bundles.dart';
...
class MyBundleCourseDetailScreen extends StatefulWidget {
  ...
  final int courseId;
  final int len;
  const MyBundleCourseDetailScreen({Key? key, required this.bundleId, required this.courseId, required this.len}) : super(key: key);

  @override
  // ignore: library_private_types_in_public_api
  _MyBundleCourseDetailScreenState createState() => _MyBundleCourseDetailScreenState();
}

class _MyBundleCourseDetailScreenState extends State<MyBundleCourseDetailScreen> with SingleTickerProviderStateMixin {
      ...
      if (status == DownloadTaskStatus.complete) {
        ...
        if (val != true) {
          await DatabaseHelper.instance.addCourse(
            CourseDbModel(courseId: courseId, courseTitle: courseTitle, thumbnail: thumbnail),
          );
        }
        var sec = await DatabaseHelper.instance.sectionExists(sectionId);
        if (sec != true) {
          await DatabaseHelper.instance.addSection(
            SectionDbModel(courseId: courseId, sectionId: sectionId, sectionTitle: sectionTitle),
          );
        }
      }
    });
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      Provider.of<MyBundles>(context, listen: false).fetchCourse(widget.bundleId, widget.courseId).then((_) {
        setState(() {
          myLoadedCourse = Provider.of<MyBundles>(context, listen: false).items;
        });
      });

      Provider.of<MyBundles>(context, listen: false).fetchCourseSections(widget.courseId).then((_) {
        setState(() {
  ...
  void lessonAction(Lesson lesson) async {
    if (lesson.lessonType == 'video') {
      if (lesson.videoTypeWeb == 'system' || lesson.videoTypeWeb == 'html5' || lesson.videoTypeWeb == 'amazon') {
        Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => PlayVideoFromNetwork(courseId: widget.courseId, lessonId: lesson.id!, videoUrl: lesson.videoUrlWeb!)),
        );
      } else if (lesson.videoTypeWeb == 'Vimeo') {
        ...
        Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => PlayVideoFromVimeoId(courseId: widget.courseId, lessonId: lesson.id!, vimeoVideoId: vimeoVideoId),
            ));
      } else {
        Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => PlayVideoFromYoutube(courseId: widget.courseId, lessonId: lesson.id!, videoUrl: lesson.videoUrlWeb!),
            ));
      }
      ...
      if (lesson.attachmentType == 'iframe') {
        ...
        Navigator.of(context).pushNamed(WebViewScreenIframe.routeName, arguments: url);
      } else if (lesson.attachmentType == 'description') {
        data = lesson.attachment;
        Navigator.push(context, MaterialPageRoute(builder: (context) => FileDataScreen(textData: data, note: lesson.summary!)));
      ...
      return RichText(
        text: const TextSpan(
          children: [
            ...
            TextSpan(text: 'Quiz', style: TextStyle(fontSize: 12, color: kSecondaryColor)),
          ],
        ),
      );
    } else {
      return RichText(
        text: const TextSpan(
          children: [
            ...
            TextSpan(text: 'Attachment', style: TextStyle(fontSize: 12, color: kSecondaryColor)),
          ],
        ),
      );
    }
                            ...
                            children: [
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    PopupMenuButton(
                                      onSelected: (value) {
                                        if (value == 'details') {
                                          Navigator.of(context).pushNamed(CourseDetailScreen.routeName, arguments: myLoadedCourse[i].id);
                                        } else {
                                          Share.share(myLoadedCourse[i].shareableLink.toString());
                                        }
                                      },
                                      icon: const Icon(
                                        Icons.more_vert,
                              ...
                              Padding(
                                ...
                                child: LinearPercentIndicator(
                                  ...
                                  percent: myLoadedCourse[i].courseCompletion! / 100,
                                  ...
                                  children: [
                                    Expanded(
                                      ...
                                      child: Padding(
                                        ...
                                        child: CustomText(
                                          text: '${myLoadedCourse[i].courseCompletion}% Complete',
                                          fontSize: 12,
                                    ...
                                    Padding(
                                      padding: const EdgeInsets.only(bottom: 10.0),
                                      child: CustomText(
                                        text: '${myLoadedCourse[i].totalNumberOfCompletedLessons}/${myLoadedCourse[i].totalNumberOfLessons}',
                                        fontSize: 14,
                                        colors: Colors.grey,
                                      ),
                                    ),
                                                  ...
                                                  child: Checkbox(
                                                      ...
                                                      onChanged: (bool? value) {
                                                        ...
                                                        setState(() {
                                                          ...
                                                          if (value) {
                                                            myLoadedCourse[i].totalNumberOfCompletedLessons =
                                                                myLoadedCourse[i].totalNumberOfCompletedLessons! + 1;
                                                          } else {
                                                            myLoadedCourse[i].totalNumberOfCompletedLessons =
                                                                myLoadedCourse[i].totalNumberOfCompletedLessons! - 1;
                                                          }
                                                          var completePerc = (myLoadedCourse[i].totalNumberOfCompletedLessons! /
                                                                  myLoadedCourse[i].totalNumberOfLessons!) *
                                                              100;
                                                          myLoadedCourse[i].courseCompletion = completePerc.round();
                                                        });
                                                ...
                                                if (lesson.lessonType == 'video')
                                                  Expanded(
                                                    ...
                                                    child: IconButton(
                                                      ...
                                                      onPressed: () => _initDownload(lesson, widget.courseId, myLoadedCourse[i].title,
                                                          myLoadedCourse[i].thumbnail, section.title, section.id),
                                                    ),
                                                  ),
                                              ],
                                            ),
                                    ...
                                    child: Checkbox(
                                        ...
                                        onChanged: (bool? value) {
                                          ...
                                          setState(() {
                                            lesson.isCompleted = value! ? '1' : '0';
                                            if (value) {
                                              myLoadedCourse[0].totalNumberOfCompletedLessons = myLoadedCourse[0].totalNumberOfCompletedLessons! + 1;
                                            } else {
                                              myLoadedCourse[0].totalNumberOfCompletedLessons = myLoadedCourse[0].totalNumberOfCompletedLessons! - 1;
                                            }
                                            var completePerc =
                                                (myLoadedCourse[0].totalNumberOfCompletedLessons! / myLoadedCourse[0].totalNumberOfLessons!) * 100;
                                            myLoadedCourse[0].courseCompletion = completePerc.round();
                                          });
                                          Provider.of<MyBundles>(context, listen: false)
                                              .toggleLessonCompleted(lesson.id!.toInt(), value! ? 1 : 0)
                                              .then((_) => CommonFunctions.showSuccessToast('Course Progress Updated'));
                                    ...
                                    Expanded(
                                      ...
                                      child: IconButton(
                                        ...
                                        onPressed: () => _initDownload(
                                            lesson, widget.courseId, myLoadedCourse[0].title, myLoadedCourse[0].thumbnail, section.title, section.id),
                                      ),
                                    ),
                                ],
                              ),
    ...
    return _isLoading
        ...
        : NestedScrollView(
                  ...
                  child: SizedBox(
                    ...
                    child: Padding(
                      ...
                      child: TabBar(
                        ...
                        indicator: BoxDecoration(borderRadius: BorderRadius.circular(8), color: kRedColor),
            ...
            body: TabBarView(
              ...
              children: [
                myCourseBodyTwo(),
                if (liveClassStatus == true) LiveClassTabWidget(courseId: widget.courseId),
                if (courseForumStatus == true) ForumTabWidget(courseId: widget.courseId),
              ],
            ),
          );
  }
