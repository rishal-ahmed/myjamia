import 'dart:convert';
import 'package:myjamia/models/bundle.dart';
import 'package:myjamia/models/bundle_courses_model.dart';
...
class MyBundles with ChangeNotifier {
    ...
    try {
      ...
      for (var bundleData in extractedData) {
        loadedBundleDetails.add(BundleDetails(
          ...
          averageRating: bundleData['average_rating'],
          numberOfRatings: bundleData['number_of_ratings'],
          bundleCourses: buildBundleCourses(bundleData['bundle_courses'] as List<dynamic>),
        ));
      }
  ...
  Future<void> fetchCourse(int bundleId, int courseId) async {
    final authToken = await SharedPreferenceHelper().getAuthToken();
    var url = '$BASE_URL/api/my_bundle_course_details/$bundleId/$courseId?auth_token=$authToken';
    try {
      final response = await http.get(Uri.parse(url));
      final extractedData = json.decode(response.body) as List;
      // ignore: unnecessary_null_comparison
  ...
  Future<void> toggleLessonCompleted(int lessonId, int progress) async {
    final authToken = await SharedPreferenceHelper().getAuthToken();
    var url = '$BASE_URL/api/save_course_progress?auth_token=$authToken&lesson_id=$lessonId&progress=$progress';
    try {
      final response = await http.get(Uri.parse(url));
