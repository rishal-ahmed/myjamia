...
class MyCourses with ChangeNotifier {
  ...
  Future<void> fetchCourseSections(int courseId) async {
    final authToken = await SharedPreferenceHelper().getAuthToken();
    var url = '$BASE_URL/api/sections?auth_token=$authToken&course_id=$courseId';
  ...
  Future<void> toggleLessonCompleted(int lessonId, int progress) async {
    final String? authToken = await SharedPreferenceHelper().getAuthToken();
    final String url = '$BASE_URL/api/save_course_progress?auth_token=$authToken&lesson_id=$lessonId';
    log(url, name: 'toggleLessonCompleted');
    try {
      final Response response = await http.get(Uri.parse(url));
      log(response.body.toString(), name: 'LessonCompleted');
      final responseData = json.decode(response.body);
      if (responseData['course  _id'] != null) {
        final myCourse = findById(int.parse(responseData['course_id']));
        myCourse.courseCompletion = responseData['course_progress'];
        myCourse.totalNumberOfCompletedLessons = responseData['number_of_completed_lessons'];
  ...
  Future<void> updateDripContendLesson(int courseId, int courseProgress, int numberOfCompletedLessons) async {
