...
class DatabaseHelper {
  ...
  Future<List<VideoModel>> getVideos() async {
    ...
    List<VideoModel> videoList = videos.isNotEmpty ? videos.map((c) => VideoModel.fromMap(c)).toList() : [];
    return videoList;
  }
  ...
  Future<List<CourseDbModel>> getCourse() async {
    ...
    List<CourseDbModel> courseList = courses.isNotEmpty ? courses.map((c) => CourseDbModel.fromMap(c)).toList() : [];
    return courseList;
  }
  ...
  Future<int> removeCourse(id) async {
    Database db = await instance.database;
    return await db.delete('course_list', where: 'course_id = ?', whereArgs: [id]);
  }
  ...
  Future<List<SectionDbModel>> getSection(id) async {
    Database db = await instance.database;
    var sections = await db.query('section_list', orderBy: 'id', where: 'course_id = $id');
    List<SectionDbModel> sectionList = sections.isNotEmpty ? sections.map((c) => SectionDbModel.fromMap(c)).toList() : [];
    return sectionList;
  }

  Future<List<Map<String, dynamic>>> queryAllSections(id) async {
    Database db = await instance.database;
    var result = await db.query('section_list', orderBy: 'id', where: 'course_id = $id');
    return result.toList();
  }
  ...
  Future<int> removeSection(id) async {
    Database db = await instance.database;
    return await db.delete('section_list', where: 'section_id = ?', whereArgs: [id]);
  }
