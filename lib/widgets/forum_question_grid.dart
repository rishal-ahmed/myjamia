...
import 'package:myjamia/screens/forum_reply_screen.dart';
...
class ForumQuestionGrid extends StatefulWidget {
  ...
  const ForumQuestionGrid({Key? key, required this.question, required this.index, required this.userId, required this.deleteQuestion})
      : super(key: key);
...
class _ForumQuestionGridState extends State<ForumQuestionGrid> {
  ...
  Future<void> toggleLike() async {
    try {
      await Provider.of<CourseForum>(context, listen: false).toggleForumQuestionVote(widget.question.id.toString());
      if (isLiked == true) {
        setState(() {
          isLiked = false;
  ...
  Widget build(BuildContext context) {
    var dt = DateTime.fromMillisecondsSinceEpoch(int.parse(widget.question.dateAdded.toString()) * 1000);
    // 12 Hour format:
                    ...
                    children: [
                      ...
                      if (widget.userId == widget.question.userId)
                        IconButton(
                          ...
                          onPressed: () {
                            _delete(widget.question.id.toString(), widget.index);
                          },
                          color: kRedColor,
                          iconSize: 20,
                          tooltip: 'Remove',
                        ...
                        await Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) => ForumReplyScreen(
                              ...
                              descrption: widget.question.description.toString(),
