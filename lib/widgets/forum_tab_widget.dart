...
import 'dart:convert';
import 'dart:io';
import 'package:myjamia/models/forum_questions_model.dart';
import 'package:myjamia/providers/course_forum.dart';
import 'package:myjamia/providers/shared_pref_helper.dart';
import 'package:myjamia/screens/search_forum.dart';
...
class _ForumTabWidgetState extends State<ForumTabWidget> {
  ...
  fetchMore() async {
    ...
    var url = '$BASE_URL/api/forum_questions/$authToken/${widget.courseId}/$_page';
    try {
      ...
      await Provider.of<CourseForum>(context, listen: false).fetchCourseQuestions(widget.courseId, _page);
      setState(() {});
      // }
    } catch (e) {
      rethrow;
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      Provider.of<CourseForum>(context, listen: false).fetchCourseQuestions(widget.courseId, _page).then((_) {
        setState(() {
          _isLoading = false;
          activeQuestions = Provider.of<CourseForum>(context, listen: false).questions;
        });
      });
    }
    _isInit = false;
  ...
  updatedQuestionList() async {
    ...
    await Provider.of<CourseForum>(context, listen: false).fetchCourseQuestions(widget.courseId, 1).then((_) {
      setState(() {
        _isLoading = false;
        activeQuestions = Provider.of<CourseForum>(context, listen: false).questions;
      });
  ...
  Container buildLoadingContainer() {
    return Container(
      ...
      child: Center(
        child: Text(listStatus == 403 ? "No More Questions" : "Loading More Questions ...",
                  ...
                  child: Column(
                    children: [
                          ...
                          onFieldSubmitted: (String value) async {
                            ...
                            if (searchValue.isNotEmpty) {
                              await Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => SearchForum(courseId: widget.courseId, keyWord: _controller.text),
                                ),
                              );
                      ...
                      Row(
                        ...
                        children: [
                          Text("${activeQuestions.length} Questions in this course."),
                          TextButton(
                            onPressed: () async {
                              //Use await and then navigate to report page
                              await Navigator.of(context).push(MaterialPageRoute(builder: (context) => AskQuestionWidget(courseId: widget.courseId)));
